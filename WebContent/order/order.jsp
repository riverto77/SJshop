...
<thead>
    ...
    <tbody>	
		...
		<tr>
			<td>
				<input type="checkbox" id="checkbox" name="checkbox" value="${order.item_name }" />
			</td>
			<td>
				${status.index+1 } <!-- 번호값 계산 -->
			</td>
			<td>
				<img class="image" src="../image/item/${order.item_image }" id="itemImage" width="200" height="120" />
			</td>
			<td>
				${order.item_name }
			</td>
			...
			<td>
				<input type="text" size=1 value="${order.amount }">
				<br><br>
				<label><input type="hidden" name="stock" value="${order.stock }"> 재고수 : ${order.stock }</label>
			</td>
			<tr>
			<td>
				<input type="hidden" name="item_price" value="${order.price }">
				<input type="hidden" name="item_image" value="${order.item_image }">
				<input type="hidden" name=item_code value="${order.item_code }">
				<input type="hidden" name=item_name value="${order.item_name }">
				<input type="hidden" name=item_qty value="${order.amount }">
		<%-- 	<td>
				<a href="CartQtyUp.item?item_name=${item.item_name }">
				<img src="../image/button/up.jpg" id="upImage" border=0 />
				</a><br>
				${cart.qty }<br>
				
				<a href="javascript:checkQty('${item.item_name }',${item.qty });">
		...
		<tr class="td">
			...
			총 금액 : <fmt:formatNumber value="${order.price * order.amount }" pattern="#,###" />원
			</td>
		</tr>
		</c:forEach>
		</tbody>
...
<tr>
<td colspan="3"><h2>구매자 정보</h2><br><br></td>
</tr>
	<tr>
	...
	<nav id="commandList">
	<br>
		<button class="button" type="submit" onclick="javascript:form.action='orderPro.ord';">구매하기</button>
	</nav>
</form>
</body>
</html>